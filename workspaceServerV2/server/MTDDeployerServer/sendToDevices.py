...
def getIPInformation(infectedIP,rootIP,startIPDevices,endIPDevices,serverIP,firstOctettOfRootIP):
    ...
    for line in out.split(b'\n'):
        ...
        if line.startswith(b'Nmap scan report'):
            ...
            print(f"alreadyUsedIP: {alreadyUsedIP}")
    ...
    print(f"This is the IP object: {ipObject}")
    return ipObject
...
def executeMTDMechanisms(ipInformation,timeToChangeBack,maxTryOfNewPorts,startOfPossiblePorts,endOfPossiblePorts):
    ...
    IPsToChangePort = ipInformation["IPsToChangePort"]
    ...
    for IPToChangePort in IPsToChangePort:
        try:
            print(f"the IPToChangePort is: {IPToChangePort}")
            PORT = 5555
            socketPoint = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            ...
            success = False
            while True:
                # get a port that is free on the machine 
                while not success:
                    print("this is newPort: ", newPort)
                    ...
                    if answer == "not taken":
                        ...
                        # remove port that is already taken on the machine 
